...
# 사무직 근로자들의 평생학습 참여동기 유형이 참여지속의도에 어떤 영향을 미치는지 알아 보고자
# 목표지향형 5문항, 활동지향형 5문항, 학습지향형 5문항, 참여지속의도 5문항으로 으로 구성된 설문을 실시하였다. 
# (data > 실습자료 > 3.단계적 회귀분석실습.sav)
...
df <- read_spss("3. 단계적 회귀분석 실습.sav")

df <- mutate(df, 목표지향성 = (목표지향성1 + 목표지향성2 + 목표지향성3 + 목표지향성4 + 목표지향성5)/5,
             활동지향성 = (활동지향성1 + 활동지향성2 + 활동지향성3 + 활동지향성4 + 활동지향성5)/5,
             학습지향성 = (학습지향성1 + 학습지향성2 + 학습지향성3 + 학습지향성4 + 학습지향성5)/5,
             참여지속의도 = (참여지속의도1 + 참여지속의도2 + 참여지속의도3 + 참여지속의도4 + 참여지속의도5))
df.a <- df[,c(24:27)]

describe(df.a)

df.a.reg <- lm(참여지속의도 ~ 목표지향성 + 활동지향성 + 학습지향성,data = df.a)  #입력방식(전 독립변수를 모두 투입)
summary(df.a.reg)
...
reg.res <- 1510.3 + 79.6 + 707.7  # 회귀계수. 회귀식에 의해 설명되는 분산 (SSR)
reg.sumsq <- 3487.6 # 회귀잔차. 회귀식에 의해 설명되지 않는 분산(SSE)
reg.res / (reg.res+ reg.sumsq) # R²

# F = 16.5414 / 0.558 = 29.64409

meansq <- (1510.27 + 79.62 + 707.71)/3  #평균제곱 값
rsd <- 11.47
meansq/rsd # F값
(131.6435 + 6.9401 + 61.6876)/3   # 동일하게 F값
...
# 입력방식을 사용한 회귀분석 결과 참여지속의도(Y) = 4.95579 + 1.88086*목표지향성 - 0.41460*활동지향성 + 2.25381*학습지향성으로 
# 나타낼 수 있으며, 참여지속의도에 영향을 미치는 변수는 (학습지향성 > 목표지향성 > 활동지향성) 의 순서로 나타남.
...
step.model1 <- stepAIC(lm(참여지속의도~활동지향성, data = df.a), direction = "both", trace = TRUE)
step.model2 <- stepAIC(lm(참여지속의도~활동지향성 + 목표지향성, data = df.a), direction = "both", trace = TRUE)
step.model3 <- stepAIC(lm(참여지속의도~활동지향성 + 목표지향성 + 학습지향성, data = df.a), direction = "both", trace = TRUE)
...
# 단계선택방식을 사용한 회귀분석 결과 참여지속의도(Y) = 4.7516 + 1.7470*목표지향성  + 2.0807*학습지향성으로 나타낼 수
# 있으며, 참여지속의도에 영향을 미치는 변수는 (학습지향성 > 목표지향성) 의 순서로 나타남.
...
# 후진방식을 사용한 회귀분석 결과 참여지속의도(Y) = 4.7516 + 1.7470*목표지향성  + 2.0807*학습지향성으로 나타낼 수
# 있으며, 참여지속의도에 영향을 미치는 변수는 (학습지향성 > 목표지향성) 의 순서로 나타남.
