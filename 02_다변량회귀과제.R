...
# 나타내며, 0과 1사이의 값을 지님.  직무책임, 직무복잡성,업무속도,관리복잡성,대인갈등,역할갈등  6개의 변수를 투입한 결과 
# R² 이 .3294으로 종속변수를 32.9%설명하고 있다.  adj R² (수정된 R²)은 독립변수의 수와 표본의 크기를 반영한 것으로 
# .3162로 나타났다. 
...
# 위 식의 결과값에서 D-W Statistic 이 더빈왓슨 값임. (1.797796). 더빈왓슨값이 2에 가까우므로 독립적이라 할 수 있음.
...
df.a.reg2 <- lm(직무탈진 ~ 업무속도 + 관리복잡성 + 대인갈등 + 역할갈등,data = df.a)
anova(df.a.reg2)

reg.res <- 52.092 + 11.462 + 13.919 + 2.255  # 회귀계수. 회귀식에 의해 설명되는 분산 (SSR)
reg.sumsq <- 173.772 # 회귀잔차. 회귀식에 의해 설명되지 않는 분산(SSE)
reg.res / (reg.res+ reg.sumsq) # R²

# F = 19.932 / 0.566 = 19.932

((52.092 + 11.462 + 13.919 + 2.255)/4)/0.566 # F값

summary(df.a.reg2) # 비표준화 계수에 의한 회귀분석 결과
...
library(lm.beta)
df.a.beta <- lm.beta(df.a.reg2)
summary(df.a.beta)


# 분석결과 비표준화계수(B)에 의해 다음과 같은 회귀식이 도출
# Y(직무탈진) = 0.32962 + 0.38947X(업무속도) + 0.13584X(관리복잡성) + 0.27557X(대인갈등) + 0.13303X(역할갈등)
# 업무속도, 관리복잡성, 대인갈등, 역할갈등 t값은 각각 6.131, 2.324, 4.108, 1.996 이고 p값은 모두 0.05 보다 작으므로
# 귀무가설은 기각되고 연구가설은 지지된다. 따라서 업무속도, 관리복잡성, 대인갈등, 역할갈등은 직무탈진에 정의 영향을 미친다고
# 볼 수 있다.   
# 독립변수 영향력의 상대적 크기는 표준화 계수를 활용한다. (독립변수들의 단위가 다르기 때문에 표준화를 시켜줌)
# 표준화 계수의 절대값이 큰 순서대로 영향력을 미친다. (업무속도 > 대인갈등 > 관리복잡성 = 역할갈등 순)
...
#표준화 잔차 표 구하기
plot(rstudent(df.a.reg2), main = "산점도")
...
durbinWatsonTest(df.a.reg2)

# 위 식의 결과값에서 D-W Statistic 이 더빈왓슨 값임. (1.703221). 



# 잔차의 정규분포 검정 

shapiro.test(df.a.reg2$residuals)

# Shapiro Wilks 검정의 유의 확률 (p-value) 이 0.05보다 커서 (0.2906) 영가설을 기각하므로, 잔차는 정규성을 가진다고
# 가정할 수 있음.
...
influencePlot(df.a.reg2, id.method="identify")
