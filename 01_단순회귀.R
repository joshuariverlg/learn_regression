...
df.a <- read_spss("3. 단순회귀분석.sav")
colnames(df.a) <- c("월","광고비","매출")
str(df.a)
...
# R² = 1226.927 / 1600.900 (= 1226.93+373.97) = 0.766
# F = 1226.927 / 46.747 = 26.246


summary(df.a.reg)

# 분석결과 비표준화계수(B)에 의해 다음과 같은 회귀식이 도출
# Y(매출) = 46.486 + 52.568X(광고비)
# t=5.123 이고 p=0.001 으로 귀무가설은 기각되고 연구가설은 지지된다. 따라서 광고비는 매출에 정의 영향을 미친다고
# 볼 수 있다.
...
# 위 식의 결과값에서 D-W Statistic 이 더빈왓슨 값임. (1.159751). 
...
# Shapiro Wilks 검정의 유의 확률 (p-value) 이 0.05보다 커서 (0.7293) 영가설을 기각할 수 없으므로, 잔차는 정규성을 가진다고
# 가정할 수 있음.
